...
var threat_light = OmniLight3D.new()
...
func _setup_ui():
	...
	start_button.custom_minimum_size = Vector2(180, 46)
	start_button.connect("pressed", Callable(self, "start_run"))
	...
	restart_button.custom_minimum_size = Vector2(220, 46)
	restart_button.connect("pressed", Callable(self, "start_run"))
	...
	hud_box.position = Vector2(14, 14)
	hud_box.custom_minimum_size = Vector2(1040, 90)
	...
	prompt_label.position = Vector2(260, 650)
	prompt_label.size = Vector2(760, 48)
...
func start_run():
	run_seed = int(Time.get_ticks_msec() % 1000000)
	rng.seed = run_seed
	playing = true
	elapsed = 0.0
	message_timer = 0.0
...
func _create_threat():
	threat = Node3D.new()
	threat.position = _cell_to_world(int(stage.size) - 2, 2) + Vector3(0, 1.25, 0)
	add_child(threat)
	...
	eye.mesh = eye_mesh
	eye.position = Vector3(0, 0.55, -0.65)
	eye.material_override = _material(Color(1, 1, 1, 1), Color(1, 1, 1, 1))
	threat.add_child(eye)
	threat_light = _add_light(threat.position, stage.palette.threat, 2.2, 20.0)
...
func _input(event):
	if event is InputEventMouseMotion and playing and Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED:
		...
		if event.keycode == KEY_ESCAPE:
			...
			_interact()
		elif event.keycode == KEY_R and not playing:
			start_run()
...
func _move_player(delta):
	...
	if input.length() > 0.0:
		...
		var next_pos = player.position + direction * speed * delta
...
func _update_threat(delta):
	var to_player = player.position - threat.position
	...
	if distance < 48.0:
		...
		var next = threat.position + dir * (3.25 * (0.65 + alert * 1.25)) * delta
		if not _collides(next):
			threat.position = next
			threat_light.position = next
		threat.look_at(Vector3(player.position.x, threat.position.y, player.position.z), Vector3.UP)
...
func _animate_items(delta):
	for entry in interactables:
		...
		node.position.y = entry.position.y + sin(elapsed * 3.0 + entry.position.x) * 0.12
...
func _best_target():
	...
	for entry in interactables:
		...
		if dot > 0.72 and (best.is_empty() or distance < best.distance):
...
func _update_hud():
	...
	var distance = player.position.distance_to(threat.position)
	...
	danger_label.add_theme_color_override("font_color", Color(0.98, 0.44, 0.52, 1) if danger > 0.7 else (Color(0.98, 0.80, 0.08, 1) if danger > 0.42 else Color(1, 1, 1, 1)))
...
func _end_run(success, text):
	...
	result_title.add_theme_color_override("font_color", stage.palette.exit if success else stage.palette.threat)
...
func _fill_audio():
	...
	var danger_freq = 92.0 if playing and player.position.distance_to(threat.position) < 16.0 else 0.0
